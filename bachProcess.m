...
for i=1:monkey_num
    ...
    switch char(name)
        case {'2','76','35','95'}
           ctrl.RT     = [ctrl_RT,RT_mean]; 
           crtl.RT_std = [crtl_RT_std,RT_std];
           crtl.errate = [crtl.errate,errate];
           
        case {'133','132','13'}
           nose.RT     = [nose_RT,RT_mean];
           nose.RT_std = [nose_RT_std,RT_std];
        case {'137','195','159','25','60','70'}
           gas.RT     = [gas_RT,RT_mean];
           gas.RT_std = [gas_RT_std,RT_std];
        case {'187','43','44'}
           stri.RT     = [stri_RT,RT_mean];
           stri.RT_std = [stri_RT_std,RT_std];
    end
end
RT     = [mean(ctrl.RT),mean(nose.RT),mean(gas.RT),mean(stri.RT)];
RT_std = [mean(crtl.RT_std),mean(nose.RT_std),mean(stri.RT_std),mean(gas.RT_std)];
figure
bar(1:4,RT)
hold on
