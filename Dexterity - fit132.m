...
fwd_time  =[]; back_time  = [];erro_trialID  = [];
...
%     stem(Edge_x,nframe)
    ...
    if apple(apple_start_end_disappear(i,3))>mostposition+5           % stable apple,means, the apple is taken back by experimenter
        ...
        grab_num =0;
        for j=time_window(1)-200:time_window(end)%apple_disappear_time(i)  % 从第i个苹果出来，到第i个苹果消失的time window
            if index_tip(j-1,1)<index_tip(j,1)&&index_tip(j,1)<= Edge_x && index_tip(j+1,1)>=Edge_x...
             ...
             % error type 1,try to grap outside of the slit
                grab_num  = grab_num+1;
                ...
                while 1
                    if index_tip(k+1,1)<index_tip(k,1)&&index_tip(k,1)<=Edge_x...
                       &&index_tip(k-1,1)>=Edge_x       %&&index_tip(k-1,1)<index_tip(k-2,1) index_tip(k+2,1)<index_tip(k+1,1)&&
                        ...
                        j=k+1;
                        fprintf(fid,'%6d %6d %6d \n',grab_num,fwd_time(end),back_time(end));
                        break
                    end
                        k=k+1;
                end
        ...
        if  grab_num<=1
            correct_trialID = [correct_trialID,i];
            correct_        = [fwd_time(end); back_time(end)];
            correct_trial   = [correct_trial',correct_]';
        else
            correct_trialID = [correct_trialID];
            correct_trial   = [correct_trial];
        end
    end
   if grab_num>1
      erro_num     = erro_num+1;
      erro_trialID = [erro_trialID,i];
   else
       
   end
end
erro_rate    = erro_num/(apple_num-erro_num);
